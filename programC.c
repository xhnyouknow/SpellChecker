...
  
/************************************************* 
  	159	* Function    : process_client() 
		160	* Description : 处理客户端连接函数 
			161	* Calls       :  
				162	* Called By   : main() 
					163	* Input       :  
						164	* Output      :  
...
  
		...
		    {  
		  ...
		  if(recvbuf[0]=='a'){
	        ...
	        strcpy(sendbuf," "); 
		  strcat(sendbuf,"art->are\n");
		  
		  } 
	 else  if(recvbuf[0]=='b'){
		 
		                    strcpy(sendbuf," ");   
		                   strcat(sendbuf,"babe->baby\n");
				   
				                     } else {   strcpy(sendbuf," ");     strcat(sendbuf,"correct\n"); } 
